...
/// Left-sliding Filter drawer: Category / Material / Style / Brand / Price.
/// Selections are staged in local temp state and only committed to
/// [ShopController] when "Apply Filters" is tapped (mirrors the old bottom
/// sheet's Apply-only UX) — closing the drawer without applying discards
...
class _ShopFilterDrawerState extends State<ShopFilterDrawer> {
  ...
  void initState() {
    ...
    // Loads regardless of how the drawer was opened — the trigger chip's
    // onTap also calls these, but Scaffold's drawer also opens via a native
    // edge-swipe gesture that bypasses onTap entirely.
