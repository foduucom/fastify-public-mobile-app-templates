...
// Ported near-verbatim from SOURCE's SupportTicketDetailsController
// (fetch/reply threading logic unchanged); restyling lives entirely in
// support_ticket_details_view.dart.
// TODO(backend-contract): confirm the `customer/support-tickets/:id` and
// `.../:id/replies` endpoints and response shapes against the live API.
class SupportTicketDetailsController extends GetxController
  ...
  Future<void> fetchChatsMessages(String id) async {
    try {
      ...
      final rawReplies = response['replies'] ??
          (response['data'] is Map ? response['data']['replies'] : null);
  ...
  Future<void> sendMessagesWithFiles({
    ...
    try {
      ...
      var response = await BasicProvider(
              "customer/support-tickets/$supportTicketId/replies")
          .postRequest(form)
          .catchError(handleError);
