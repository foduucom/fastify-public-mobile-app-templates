...
class LocalStorageNotificationService extends GetxService {
    ...
    try {
      ...
      if (stored != null && stored is List) {
        notifications.assignAll(
          stored
              .map((e) => LocalNotification.fromJson(Map<String, dynamic>.from(e as Map)))
              .toList(),
        );
      }
    } catch (e) {
  ...
  Future<void> _saveToStorage() async {
    try {
      await _box.write(_storageKey, notifications.map((e) => e.toJson()).toList());
    } catch (e) {
  ...
  Future<void> saveNotification(LocalNotification newNotif) async {
    ...
    if (existingIndex != -1) {
      ...
      final mergedNotif = newNotif.copyWith(
        ...
        isSynced: (existing.isRead == newNotif.isRead) ? existing.isSynced : false,
      );
  ...
  List<LocalNotification> getPaginatedNotifications(int limit, int offset) {
    ...
    final end = (offset + limit) > notifications.length ? notifications.length : (offset + limit);
  ...
  Future<void> markAsRead(String id) async {
    ...
    if (index != -1 && !notifications[index].isRead) {
      notifications[index] = notifications[index].copyWith(
        isRead: true,
        isSynced: false, // Mark as unsynced so the read status updates to backend
      );
      await _saveToStorage();
    }
  }
  ...
  Future<void> updateSyncStatus(String id, bool isSynced, {int? retryCount, DateTime? lastSyncAttempt}) async {
    final index = notifications.indexWhere((n) => n.id == id);
    if (index != -1) {
      notifications[index] = notifications[index].copyWith(
        isSynced: isSynced,
        syncRetryCount: retryCount ?? notifications[index].syncRetryCount,
        lastSyncAttempt: lastSyncAttempt ?? notifications[index].lastSyncAttempt,
      );
      await _saveToStorage();
    }
  }
